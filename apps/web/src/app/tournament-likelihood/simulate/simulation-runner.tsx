...
import { parseSimulationSettings, type SimulationSettings } from "@/lib/simulation-settings";
...
export function SimulationRunner({
  ...
  const [submittedSettings, setSubmittedSettings] = useState<SimulationSettings | null>(null);
  useEffect(() => () => {
    sourceRef.current?.close();
    sourceRef.current = null;
  }, []);
  const selectedRunSeconds = submittedSettings?.runSeconds ?? normalizeRunSeconds(runSeconds, defaultRunSeconds);
  ...
  const selectedTopCut = submittedSettings?.topCut ?? Number(topCut || defaultTopCut);
  const selectedDropAfterRound = submittedSettings ? submittedSettings.dropAfterRound : normalizeOptionalInteger(dropAfterRound, 1);
  ...
  function startSimulation() {
    ...
    try {
      settings = parseSimulationSettings({ swissRounds, topCut, runSeconds, dropAfterRound, dropMinPoints }, {
        swissRounds: defaultSwissRounds, topCut: defaultTopCut, runSeconds: defaultRunSeconds,
        dropAfterRound: defaultDropAfterRound, dropMinPoints: defaultDropMinPoints,
      });
    } catch (error) {
    ...
    const params = new URLSearchParams({
      tournament: slug,
      swissRounds: String(settings.swissRounds),
      topCut: String(settings.topCut),
      runSeconds: String(settings.runSeconds),
    });
    if (settings.dropAfterRound !== null && settings.dropMinPoints !== null) {
      params.set("dropAfterRound", String(settings.dropAfterRound));
      params.set("dropMinPoints", String(settings.dropMinPoints));
    }
    ...
    source.onmessage = (event) => {
      ...
      let nextSnapshot: SimulationSnapshot;
      try { nextSnapshot = JSON.parse(event.data) as SimulationSnapshot; }
      catch { setError("Simulation returned an invalid response."); setIsRunning(false); source.close(); return; }
      ...
      <Card className="knd-panel mt-8">
        ...
        <CardContent>
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            ...
            {submittedSettings?.swissRounds ?? (swissRounds || defaultSwissRounds)} Swiss rounds ·{" "}
            {selectedTopCut > 0 ? `Cut to Top ${selectedTopCut}` : "No top cut"} ·{" "}
            {hasDropRule
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        ...
        {hasByePointLine(selectedTopCut) && (
          <Card className="knd-panel">
            <CardHeader>
              <CardTitle className="text-sm uppercase tracking-[0.3em] text-muted-foreground">
                {byePointLineTitle(String(selectedTopCut), defaultTopCut)}
              </CardTitle>
            </CardHeader>
            <CardContent>
