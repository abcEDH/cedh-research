...
import { extractTournamentSlug, fetchTournamentBySlug, fetchTournamentStructureDefaults, defaultTournamentStructureForPlayerCount } from "@/lib/topdeck";
...
export default async function TournamentSimulationPage({
  ...
  let playerCount = 0;
  let structure = defaultTournamentStructureForPlayerCount(0);
  ...
  if (slug) {
    try {
      ...
      structure = await fetchTournamentStructureDefaults(slug, playerCount);
    } catch (error) {
      settingsError = (error as Error).message;
    }
  }

  const swissRounds = requestedSwissRounds ?? structure.swissRounds;
  const topCut = requestedTopCut ?? structure.topCut;
