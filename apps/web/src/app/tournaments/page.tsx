...
import { TIER_MIN, tournamentSummaries, type EventTier } from "@/lib/tournaments";
...
function initialSort(): SortOption {
  ...
  return value === "Players" ? "Players" : "Date";
}

function initialTier(): TierOption {
...
function initialPeriod(): PeriodOption {
  if (typeof window === "undefined") return "3 Months";
...
export default function TournamentsPage() {
  ...
  // ---- Compute rows ----
  const sizeThresh = TIER_MIN[tierFilter];
  const periodDays = PERIOD_DAYS[period];

  const items = tournamentSummaries.map((t) => {
    const d = new Date(t.date + "T00:00:00");
    const days = Math.round((TODAY.getTime() - d.getTime()) / 86400000);
    return { ...t, d, days };
  })
    .filter((t) => t.players >= sizeThresh && t.days <= periodDays)
    .sort((a, b) => (sortBy === "Players" ? b.players - a.players : a.days - b.days));

  const tierLabel =
    tierFilter === "All Tiers" ? "all tiers" : `${tierFilter.toLowerCase()} & up`;
