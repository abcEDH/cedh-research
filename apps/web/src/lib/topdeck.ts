...
export function inferTournamentStructureFromText(textOrHtml: string, playerCount = 0): TournamentStructureDefaults | null {
  ...
  return {
    swissRounds: swissRounds ?? defaultTournamentStructureForPlayerCount(playerCount).swissRounds,
    topCut: topCut ?? defaultTournamentStructureForPlayerCount(playerCount).topCut,
    source: "event_page",
  };
}
...
export async function fetchTournamentStructureDefaults(
  ...
  for (const url of [
    ...
    if (response.ok) {
      const inferred = inferTournamentStructureFromText(await response.text(), playerCount);
