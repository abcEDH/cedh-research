"""Integration test for get_winrate_matrix (#147) against a real Postgres.
...
per the repo's "Optional Dependency Detection" convention (see AGENTS.md) --
    ...
    uv run python -m unittest packages.backend.tests.test_winrate_matrix_integration
...
This exact fixture and these exact expected values were run by hand against a local Postgres 16
during development of #147 (5 commanders, 6 games including an all-draw pod and a low-frequency
commander to exercise the metashare cutoff); see the migration's module header for the
reconciliation notes that run proved out.
"""
...
# test_rebuild_player_commander_profiles.py (another module in this suite) sets
# sys.modules["psycopg2"] = None at import time, to exercise supabase_client's ImportError
# fallback path, and never restores it -- a pre-existing test-isolation gap, unrelated to #147.
# That sentinel makes both importlib.util.find_spec("psycopg2") and a plain `import psycopg2`
# report "unavailable" for every module discovered afterward in the same process, even when
# psycopg2 is genuinely installed (Python's documented behavior for a None entry in
# sys.modules). Drop the sentinel (and any already-cached submodule entries) before our own
# check, so this module isn't at the mercy of unittest discover's file ordering.
...
INSERT INTO players (id, topdeck_id, name) VALUES
  ('00000000-0000-0000-0000-000000000101', 'wm-p1', 'Player1'),
  ('00000000-0000-0000-0000-000000000102', 'wm-p2', 'Player2'),
  ('00000000-0000-0000-0000-000000000103', 'wm-p3', 'Player3'),
  ('00000000-0000-0000-0000-000000000104', 'wm-p4', 'Player4'),
  ('00000000-0000-0000-0000-000000000105', 'wm-p5', 'Player5')
ON CONFLICT (id) DO NOTHING;

INSERT INTO tournaments (id, topdeck_tid, name, start_date, player_count, swiss_rounds) VALUES
  ('00000000-0000-0000-0000-0000000000f1', 'wm-t01', 'Fixture Cup', NOW() - INTERVAL '10 days', 32, 6)
ON CONFLICT (id) DO NOTHING;
...
class WinrateMatrixIntegrationTests(unittest.TestCase):
    ...
    def test_winrate_matrix_matches_hand_computed_fixture(self) -> None:
        ...
        # Row bound: N*(N+1) for the top-N=30 in #147's acceptance criterion; here N=4 so the
        # bound is 20, and this fixture hits it exactly (every top-N pair co-occurred at least
        # once, so no empty cells to omit).
        self.assertEqual(len(rows), 20)

        pairwise_ab = by_pair[(a, b)]
        self.assertEqual(pairwise_ab["games_played"], 6)
        self.assertEqual(pairwise_ab["wins"], 2)
        self.assertEqual(pairwise_ab["losses"], 3)
        self.assertEqual(pairwise_ab["draws"], 1)
    ...
    def test_winrate_matrix_pairwise_cells_reconcile_with_get_commander_matchups(self) -> None:
        a = "00000000-0000-0000-0000-00000000000a"
        ...
        # p_min_games=0 disables get_commander_matchups' own significance floor (default 5) so
        # every co-participation is comparable -- see the migration's module header for why it's
        # the pairwise cells, not the OVERALL row, that reconcile against this RPC.
        ...
        self.assertTrue(matrix_rows, "expected at least one pairwise row for commander A")
    ...
    def test_top_n_cutoff_excludes_the_low_metashare_commander(self) -> None:
        matrix_ids = {
    ...
    def test_empty_cells_are_omitted_not_returned_with_null_winrate(self) -> None:
        rows = self._rows("SELECT * FROM public.get_winrate_matrix(%s, %s)", (4, 365))

        for row in rows:
            self.assertIsNotNone(row["point_winrate"])
            self.assertGreater(row["games_played"], 0)
