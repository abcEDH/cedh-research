...
class ActiveCommanderMlTests(unittest.TestCase):
    def test_split_never_splits_a_target_date(self):
        examples = [
            {"target_date": date(2026, 1, day), "topdeck_id": str(i)} for i, day in enumerate([1, 1, 2, 2, 2, 3])
        ]
        train, test = ml.split_examples_by_time(examples, train_fraction=0.6)
        self.assertLess(max(row["target_date"] for row in train), min(row["target_date"] for row in test))
        with self.assertRaises(ValueError):
            ml.split_examples_by_time(examples[:2], train_fraction=0.5)

    def test_same_date_elo_events_follow_replay_order(self):
        example = {"topdeck_id": "p", "target_date": date(2026, 2, 1), "history_rows": [{"player_id": "p"}]}
        events = [
            {"game_date": "2026-01-01", "tournament_id": "t", "game_id": "g2", "round_number": 2, "rating_after": 1520},
            {"game_date": "2026-01-01", "tournament_id": "t", "game_id": "g1", "round_number": 1, "rating_after": 1510},
            {"game_date": "2026-02-01", "game_id": "future", "rating_after": 9999},
        ]
        result = ml.enrich_examples_with_hidden_elo([example], {"p": events})[0]
        self.assertEqual(result["hidden_elo_before"], 1520)
        self.assertEqual(result["hidden_elo_games_before"], 2)

    def test_ml_bucket_report_keeps_enriched_features(self):
        examples = [
            {
                "topdeck_id": "p",
                "target_date": date(2026, month, 1),
                "actual": "A",
                "history_rows": [usage_row("A", "2025-01-01")],
                "hidden_elo_before": 1800 + month,
                "hidden_elo_games_before": 100 + month,
            }
            for month in [1, 2, 3]
        ]
        seen = []
        ...
        def predict(model, example):
            seen.append(example)
            self.assertGreater(example["hidden_elo_before"], 1800)
            self.assertGreater(example["hidden_elo_games_before"], 100)
            return {"A": 1.0}

        with (
            patch.object(ml, "train_model", return_value=(object(), {})),
            patch.object(ml, "predict_ml_distribution", side_effect=predict),
        ):
            report = ml.run_ml_backtest(examples, model_type="logistic", train_fraction=0.5)
        self.assertTrue(seen)
        self.assertTrue(all(row["log_loss"] == 0 for row in report["bucket_report"].values()))
