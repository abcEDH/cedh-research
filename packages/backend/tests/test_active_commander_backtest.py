...
import commander_history_client as history_client  # noqa: E402
...
class ActiveCommanderBacktestTests(unittest.TestCase):
    def test_rest_fallback_diagnostics_do_not_pollute_json_stdout(self):
        stdout, stderr = io.StringIO(), io.StringIO()
        with (
            patch.dict(
                history_client.os.environ,
                {
            ...
            patch.object(history_client, "load_local_env"),
            patch.object(history_client, "fetch_usage_rows_via_db", side_effect=RuntimeError("unavailable")),
            patch.object(history_client, "create_client") as create,
            patch.object(history_client, "fetch_usage_rows_via_rest", return_value=[]),
            redirect_stdout(stdout),
            redirect_stderr(stderr),
        ):
            self.assertEqual(history_client.load_usage_rows(), [])
        self.assertEqual(stdout.getvalue(), "")
        self.assertIn("falling back to REST", stderr.getvalue())
    ...
    def test_load_usage_rows_uses_scoped_db_query_for_windowed_backtest(self) -> None:
        with (
            ...
            patch.object(history_client, "fetch_usage_rows_via_db") as full_fetch,
        ):
