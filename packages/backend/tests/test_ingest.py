...
class CommanderNormalizationTests(unittest.TestCase):
    ...
    def test_deduplicate_commanders_keeps_single_partner_pair(self) -> None:
        ...
        result, aliases = deduplicate_commanders_in_batch(commander_data)
    ...
    def test_deduplicate_commanders_merges_reversed_partner_pairs(self) -> None:
        ...
        self.assertIn("Tymna the Weaver / Kraum, Ludevic's Opus", result)
        # The discarded reversed-order name must still resolve to the kept name,
        # so standings keyed by the discarded variant don't silently lose their
        # commander_id lookup.
        self.assertEqual(
            aliases,
            {"Kraum, Ludevic's Opus / Tymna the Weaver": "Tymna the Weaver / Kraum, Ludevic's Opus"},
        )
...
class BatchUpsertCommandersTests(unittest.TestCase):
    def test_discarded_pair_order_alias_still_resolves_to_kept_commander_id(self) -> None:
        # Regression test: when both name-order variants of the same partner
        # pair appear in one ingestion batch, deduplicate_commanders_in_batch
        # only upserts one of them, but standings keyed by the *discarded*
        # variant must still resolve to that same commander_id instead of
        ...
        client.upsert.return_value = [
            {"name": "Tymna the Weaver / Kraum, Ludevic's Opus", "id": "commander-1"}
        ]
        ingester = DataIngester(topdeck=Mock(), supabase=client)

        result = ingester.batch_upsert_commanders(commander_data)

        # Only one row should have been upserted (deduplicated)...
        self.assertEqual(len(client.upsert.call_args.args[1]), 1)
        # ...but both original name-order keys must resolve to that row's id.
        self.assertEqual(
            result,
            {
                "Tymna the Weaver / Kraum, Ludevic's Opus": "commander-1",
                "Kraum, Ludevic's Opus / Tymna the Weaver": "commander-1",
            },
        )
