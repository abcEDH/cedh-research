-- Future tournaments remain ingested, but cannot be read as Elo games.
-- Use the full timestamp, including future times on the current UTC date.
-- Ranking eligibility is a game-level rule. Every participant in an eligible
-- game must remain available to Elo scoring and matchup aggregation, even if
-- an individual entry has no decklist.
...
-- NOT VALID allows installing protection before repairing existing bad events.
-- New inserts/updates are checked immediately; recovery validates after replay.
ALTER TABLE public.global_elo_game_events
  ADD CONSTRAINT global_elo_game_events_no_future_date
  CHECK (game_date <= CURRENT_TIMESTAMP) NOT VALID;
