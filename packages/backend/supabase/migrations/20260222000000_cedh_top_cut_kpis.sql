-- Migration: Consolidated cEDH top-cut KPI view (Top 4/10/16)
-- Purpose:
--   Single view for per-commander top-cut metrics across cEDH cut structures.
--   Includes:
--   - Total decks analyzed
--   - Top 4/10/16 appearance share (share of all top slots filled)
--   - Top 4/10/16 conversion rate (appearances / entries in that event bucket)
--   - Top 4/10/16 win rate (event wins / top-cut appearances)

DROP VIEW IF EXISTS commander_top_cut_kpis;
...
GRANT SELECT ON commander_top_cut_kpis TO anon;
GRANT SELECT ON commander_top_cut_kpis TO authenticated;
GRANT SELECT ON commander_top_cut_kpis TO service_role;
