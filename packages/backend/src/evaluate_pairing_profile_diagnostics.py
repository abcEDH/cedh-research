...
from evaluate_topdeck_pairings import (
    ...
    pod_record_profile,
    profile_recall,
    set_round_eligibility,
)
from ingest import load_local_env
from pairing_history_client import fetch_round_rows
...
def round_diagnostics(state: TournamentState, actual_pods: list[Pod]) -> dict[str, Any]:
    ...
    point_ranges = [(max(profile) - min(profile)) if profile else 0 for profile in actual_points]
    ...
    eligible = state.eligible_player_ids if state.eligible_player_ids is not None else set(state.standings)
    return {
