...
from pairing_history_client import HistoricalRound, build_historical_rounds, fetch_round_rows, set_round_eligibility
from supabase import Client
from supabase_client import get_supabase_client

__all__ = ["HistoricalRound", "build_historical_rounds", "fetch_round_rows", "set_round_eligibility"]
from run_historical_tournament_from_round_sim import fetch_seat_map
...
def build_standings_order_repeat(state: TournamentState, round_index: int, rng: random.Random) -> list[Pod]:
    ...
    if repeat_avoidance_max_pods and len(pod_groups) <= min(32, repeat_avoidance_max_pods):
...
def build_points_seed_order(state: TournamentState, round_index: int, rng: random.Random) -> list[Pod]:
    ...
    eligible = state.eligible_player_ids
    ordered = sorted(
        (player_id for player_id in state.standings if eligible is None or player_id in eligible),
...
def aggregate_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for candidate, metric_rows in sorted(by_candidate.items()):
        ...
        aggregate[candidate] = {
            ...
            "average": {key: statistics.fmean(row[key] for row in metric_rows) for key in metric_keys},
            "by_round": {
                str(round_number): {key: statistics.fmean(row[key] for row in round_metric_rows) for key in metric_keys}
...
def fetch_candidate_tournaments(
    ...
    *,
    ...
    rows = query.limit(candidate_scan_limit).execute().data or []
...
def evaluate_tournament(
    ...
    *,
    ...
    for round_number in sorted(rounds):
        ...
        set_round_eligibility(state, historical_round)
...
def main() -> None:
    ...
    all_round_rows = [row for result in results for row in result["rounds"]]
    payload = {
        "config": {
            "samples": args.samples,
            "repeat_avoidance_max_pods": args.repeat_avoidance_max_pods,
