...
This command only reports suspected duplicates. Matching player sets/table numbers
is not enough to authorize deletion: outcomes and downstream Elo also need review.
"""
...
def posted_tables_from_topdeck(tournament: dict) -> list[PostedTable]:
    ...
    for round_data in tournament.get("rounds") or []:
        ...
        for fallback_table_number, table in enumerate(round_data.get("tables") or [], start=1):
            players = frozenset(str(player.get("id")) for player in table.get("players") or [] if player.get("id"))
            if len(players) < 2:
                continue
...
def audit_tournament(
    topdeck_client: TopDeckClient, supabase_client: SupabaseClient, topdeck_tid: str
) -> TournamentAudit:
...
def print_audit(audit: TournamentAudit, *, show_samples: int) -> None:
    ...
    for game in audit.duplicate_delete_candidates[:show_samples]:
        print(
            "    delete_candidate "
            f"game_id={game.game_id} round={game.round_number} table={game.table_number} "
            f"players={len(game.player_topdeck_ids)}"
        )
    ...
    for key, games, posted in audit.unsafe_duplicate_groups[:show_samples]:
        print(
            ...
            f"stored_tables={[game.table_number for game in games]} "
            f"posted_tables={[table.table_number for table in posted]}"
        )
