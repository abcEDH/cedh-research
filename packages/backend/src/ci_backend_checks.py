...
def resolve_benchmark_fixture(supabase_url: str, headers: dict[str, str]) -> BenchmarkFixture:
    ...
    if regional_resp.status_code != 200:
        raise RuntimeError(f"Unable to resolve benchmark regional fixture: {response_failure(regional_resp)}")
    ...
    for spec in specs:
        ...
        try:
            for _ in range(warmups):
                _elapsed_ms, data = _run_benchmark_request(session, request, headers)
                if not data:
