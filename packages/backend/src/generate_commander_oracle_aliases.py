...
"legal two-card pairing" rules, this pulls every printing (including UB
flavor names) of every commander-eligible card from Scryfall's search API
(``is:commander``, ``unique=prints`` -- roughly 12k printings, versus ~550MB
for the full ``default_cards`` bulk dataset) and records, for every
commander-legal card that has ever been printed under an alternate flavor
name, the mapping from that flavor name back to the true Oracle name sharing
its ``oracle_id``.
...
from commander_oracle_identity import build_alias_map
from generate_legal_commander_pairings import SCRYFALL_SEARCH_URL, fetch_commander_eligible_prints
...
def write_output(alias_map: dict[str, str], *, output_path: Path, timeout: float) -> None:
    payload = {
        ...
        "source": {
            "provider": "Scryfall",
            "dataset": "commander_eligible_prints",
            "search_url": SCRYFALL_SEARCH_URL,
            "search_query": "is:commander",
...
def main() -> None:
    ...
    cards = fetch_commander_eligible_prints(args.timeout)
