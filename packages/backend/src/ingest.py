...
from commander_dedup import resolve_partner_order_conflicts
from commander_normalization import (
...
class DataIngester:
    ...
    def batch_upsert_commanders(self, commander_data: dict[str, list[str]]) -> dict[str, str]:
        ...
        # Canonicalize first and de-dupe by the resulting name: two different
        # raw dict keys (e.g. "A / B" and "B / A" for the same partner pair)
        # must collapse to a single payload, or a single multi-row upsert
        # would try to affect the same on_conflict target twice and Postgres
        # would reject it.
        payloads: dict[str, list[str]] = {}
        for name, names in commander_data.items():
            canonical_name, canonical_names = sanitize_commander_payload(name, names)
            payloads[canonical_name] = canonical_names

        # Self-heal any pre-existing rows stored under the *other* partner
        # order for a pair in this batch (issue #260) before the upsert below
        # would otherwise create a second, canonical-named row alongside a
        # stale non-canonical one.
        resolve_partner_order_conflicts(self.supabase, payloads)

        data = [{"name": name, "commander_names": names} for name, names in payloads.items()]
