...
def build_candidate_rows(example: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    distinct_prior_tournaments = len({row.get("tournament_id") for row in history_rows if row.get("tournament_id")})
...
def split_examples_by_time(
    ...
    *,
    ...
    boundaries = [
        ...
        if sorted_examples[i - 1]["target_date"] < sorted_examples[i]["target_date"]
    ]
    if not boundaries:
        raise ValueError("Chronological validation needs at least two distinct target dates")
    desired = len(sorted_examples) * train_fraction
    split_index = min(boundaries, key=lambda i: (abs(i - desired), i))
...
def fetch_hidden_elo_events_for_examples(
    ...
    sql = """
        SELECT e.player_id, e.game_date, e.rating_before, e.rating_after,
               e.game_id, e.tournament_id, g.round_number, g.round_name, g.table_number
        FROM global_elo_game_events e
        JOIN games g ON g.id = e.game_id
        WHERE e.region_type = 'global'
          AND e.region_key = 'ALL'
          AND e.player_id = ANY(%s::uuid[])
          AND e.game_date < %s
        ORDER BY e.player_id ASC, e.game_date ASC
    """
...
def enrich_examples_with_hidden_elo(
    ...
    # Replay order, including Swiss before cut and numeric cut-round order.
    events_by_player = {
        player_id: sorted(
            [row for row in rows if baseline.parse_start_datetime(row.get("game_date")) is not None],
            key=lambda row: game_sort_key(
                (
                    ...
                    [
                        {
                            **row,
                            "start_date": baseline.parse_start_datetime(row["game_date"]).isoformat(),
                        }
                    ],
                )
            ),
        )
        for player_id, rows in events_by_player.items()
    }
...
def ensemble_sweep(model: Any, examples: list[dict[str, Any]], weights: list[float]) -> dict[str, dict[str, Any]]:
    # Trial mixtures: 0%, 10%, ..., 100% ML, with the remainder from production.
    # These are blend coefficients, not player decile buckets; final selection
    # would require a separate holdout beyond this exploratory sweep.
    ...
    *,
    ...
    output = {
        ...
        "bucket_report": baseline.run_bucket_backtest(
            test_examples,
            predict_example=lambda example: predict_ml_distribution(model, example),
        ),
    }
