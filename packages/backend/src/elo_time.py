...
def exclude_future_games(
    ...
    *,
    ...
    future_ids = {
        row["game_id"]
        for row in rows
        if row.get("game_id") and row.get(date_key) and utc_datetime(row[date_key]) > cutoff
    }
    return [
        row
        for row in rows
        if row.get("game_id") not in future_ids and (not row.get(date_key) or utc_datetime(row[date_key]) <= cutoff)
    ]
