...
def size_bucket(player_count: int) -> str:
    """Exploratory field-size bins (<40, 40–95, 96+), not TopDeck medal tiers.

    These inherited reporting cutoffs have not been fitted as pairing thresholds.
    Actual repeat avoidance uses the separate 32-pod limit.
    """
    if player_count < 40:
        return "small"
    if player_count < 96:
