...
def split_tournaments(
    ...
    *,
    ...
    for tournament_ids in by_bucket.values():
        ...
        train_count = (
            max(1, min(len(shuffled) - 1, round(len(shuffled) * train_fraction)))
            if len(shuffled) > 1
            else len(shuffled)
        )
...
def learn_policy(rows: list[dict[str, Any]], metric: str) -> dict[str, Any]:
    ...
    policy = {
        ...
        "by_size": {size: best_candidate(group_rows, metric) for size, group_rows in sorted(by_size.items())},
...
def policy_candidate(policy: dict[str, Any], row: dict[str, Any]) -> str | None:
    ...
    return policy["by_size_round"].get(f"{size}:{round_number}") or policy["by_size"].get(size) or policy.get("global")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--metric", default="pair_recall", choices=["pair_recall", "pair_recall_best", "exact_pod_recall"]
    )
