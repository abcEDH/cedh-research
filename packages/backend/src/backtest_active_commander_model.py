...
from commander_history_client import (
    fetch_usage_rows_for_target_window_via_db,
    load_usage_rows,
    normalize_usage_start_dates,
)
from commander_model_config import (
    DECKLIST_SHARE_WEIGHT,
    LATEST_BONUS_WEIGHT,
    LIFETIME_SHARE_WEIGHT,
    RECENT_SHARE_WEIGHT,
    REPEAT_BONUS_WEIGHT,
)
...
__all__ = ["fetch_usage_rows_for_target_window_via_db", "load_usage_rows", "normalize_usage_start_dates"]
...
def most_played_distribution(
    history_rows: list[dict[str, Any]], _reference_date: date | None = None
) -> dict[str, float]:
...
def hybrid_distribution(history_rows: list[dict[str, Any]], reference_date: date) -> dict[str, float]:
    ...
    for commander in commanders:
        ...
        scores[commander] = (
            RECENT_SHARE_WEIGHT * recent_share.get(commander, 0.0)
            + LIFETIME_SHARE_WEIGHT * lifetime_share.get(commander, 0.0)
            + LATEST_BONUS_WEIGHT * latest_bonus
            + REPEAT_BONUS_WEIGHT * repeat_bonus
            + DECKLIST_SHARE_WEIGHT * decklist_share.get(commander, 0.0)
        )
    return normalize_scores(scores)
...
def latest_blend_distribution(
    ...
    *,
    ...
    return normalize_scores(
        {
            commander: ((1 - latest_weight) * current.get(commander, 0.0) + latest_weight * latest.get(commander, 0.0))
            for commander in commanders
        }
    )
    ...
    *,
    ...
    for example in examples:
        distribution = (
            ...
            if predict_example is not None
            else predict(example["history_rows"], example["target_date"])
        )
